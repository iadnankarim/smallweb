...
/** Plain text from HTML, good enough for search and snippets. Works on server and client. */
...
export function tokenize(query: string): string[] {
  return Array.from(new Set(query.toLowerCase().split(/[^a-z0-9’']+/).filter((t) => t.length > 1)));
}
...
export function countOccurrences(haystack: string, term: string): number {
  const matches = haystack.toLowerCase().match(new RegExp(`\\b${escapeRe(term)}`, "g"));
  return matches ? matches.length : 0;
}

/** A window of text around the first hit, split into highlighted parts. */
export function buildSnippet(text: string, terms: string[], radius = 90): SnippetPart[] {
  const lower = text.toLowerCase();
  const first = terms.map((t) => lower.search(new RegExp(`\\b${escapeRe(t)}`))).filter((i) => i >= 0);
